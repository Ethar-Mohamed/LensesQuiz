<template>
  ...
  >
    ...
    <div class="text-center mb-8 z-10">
      ...
      <p class="text-xl italic text-slate-700 mt-4">{{ $t("typequestion") }}</p>
    </div>
    ...
    >
      <div
        v-for="(option, index) in wouldyouwearOptions"
        :key="index"
...
export default {
  ...
  data() {
    return {
      ...
      errorMessage: "",
      wouldyouwearOptions: [
        { label: "A", text: "👁️ Daily basis", value: "Daily basis" },
        { label: "B", text: "👨‍💻 At the office", value: "At the office" },
        { label: "C", text: "🎉 At parties", value: "At parties" },
        { label: "D", text: "💕 For dates", value: "For dates" },
      ],
    };
  },
  computed: {
  ...
  methods: {
    toggleOption(option) {
      ...
      this.errorMessage = this.selected.length ? "" : this.errorMessage;

      localStorage.setItem("wouldyouwear", JSON.stringify(this.selected));
    },
    submit() {
      if (this.selected.length) {
        localStorage.setItem("whenWearCode", JSON.stringify(this.selected));
        this.$router.push({
          name: "QuestionSix",
        });
      } else {
