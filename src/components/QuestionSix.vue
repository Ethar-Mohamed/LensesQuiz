<template>
  ...
  >
    ...
    <div class="text-center mb-8 z-10">
      ...
      <p class="text-xl italic text-slate-700 mt-4">
        {{ $t("typequestion6") }}
      </p>
    </div>
    ...
    >
      <div
        v-for="(option, index) in lensesmatchOptions"
        :key="index"
      ...
      >
        ...
        <div class="mt-2 text-sm font-medium text-slate-700">
          ...
          <span class="text-base mx-1">{{ $t(`option.${option.value}`) }}</span>
        </div>
      </div>
    </div>
  ...
  data() {
    return {
      ...
      lensesmatchOptions: [
        ...
        { label: "B", text: "🧕 Hijab ", value: "Hijab" },
        { label: "C", text: "🤷‍♀️ No preference ", value: "No preference" },
      ],
    };
  },
  methods: {
    selectOption(option) {
      ...
      localStorage.setItem("lensesmatch", this.selected);
